...
/// Kind 9 — reply carrying an interactive prompt response.
///
/// Deliberately narrow: the caller supplies only `(request event id, prompt id,
/// option id)` and the p-tagged requesting agent. Content is descriptive text
/// generated by the caller from the request's own option label — never
/// executable command text — and no other tags can be attached.
///
/// Emits both `root` and `reply` markers for the request event, per the prompt
/// v1 contract: the requesting agent verifies the response threads to exactly
/// the request it registered.
pub fn build_prompt_response(
    channel_id: Uuid,
    ...
    if content.trim().is_empty() {
        return Err("prompt response content must not be empty".into());
    }
    if !crate::prompt::KNOWN_OPTION_IDS.contains(&option_id) {
        return Err(format!("unknown prompt option id: {option_id}"));
    }
    check_pubkey(agent_pubkey)?;
    let request_hex = request_event_id.to_hex();
    let tags = vec![
        tag(vec!["h", &channel_id.to_string()])?,
        tag(vec![
            crate::prompt::PROMPT_RESPONSE_TAG,
            crate::prompt::PROMPT_VERSION,
            prompt_id,
            option_id,
        ])?,
        tag(vec!["e", &request_hex, "", "root"])?,
        tag(vec!["e", &request_hex, "", "reply"])?,
        tag(vec!["p", &agent_pubkey.to_ascii_lowercase()])?,
    ];
    Ok(EventBuilder::new(Kind::Custom(9), content).tags(tags))
}
